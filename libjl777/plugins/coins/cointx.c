...
struct cointx_info *_decode_rawtransaction(char *hexstr,int32_t oldtx)
{
    ...
    uint32_t vin,vout;
if ( hexstr[strlen(hexstr)-1] == '\n' || hexstr[strlen(hexstr)-1] == '\r' )
hexstr[strlen(hexstr)-1] = 0;
    if ( (len= strlen(hexstr)) >= sizeof(data)*2-1 || is_hexstr(hexstr) == 0 || (len & 1) != 0 )
    {
        printf("_decode_rawtransaction.(%s): oldtx.%d hexstr too long %ld vs %ld || is_hexstr.%d || oddlen.%ld\n",hexstr,oldtx,strlen(hexstr),sizeof(data)*2-1,is_hexstr(hexstr),(len & 1));
        return(0);
    }
